            ...
            <section>
                {sortMethods.map((method, index) => (
                    <label key={index} htmlFor={method}>
                        {method}
                        <input type="radio" id={method} name="sortMethod" value={method} defaultChecked={index === 0 ? true : false} onChange={(event) => handleSortMethodChange(event.target.value as )} ></input>
                    </label>
                ))}
            </section>
